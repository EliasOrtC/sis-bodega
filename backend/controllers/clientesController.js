...
exports.addCliente = async (req, res) => {
    try {
        ...
        const clienteIdResult = await db.execute('SELECT last_insert_rowid() as id');
        const clienteId = clienteIdResult.rows[0][0];
        await logAction(userId, username, 'Clientes', 'INSERT', `(ID${clienteId}) Cliente agregado: ${nombres} ${apellidos}`);
        scanner.notificarClientes();
        res.status(201).json({ message: `(ID${clienteId}) Cliente agregado correctamente` });
    } catch (error) {
...
exports.updateCliente = async (req, res) => {
    try {
        ...
        if (oldResult.rows.length > 0) {
            ...
            await logAction(userId, username, 'Clientes', 'UPDATE', `(ID${id}) Cliente actualizado: ${nombres} ${apellidos} | ${diff}`);
        }
        ...
        scanner.notificarClientes();
        res.json({ message: `(ID${id}) Cliente actualizado correctamente` });
    } catch (error) {

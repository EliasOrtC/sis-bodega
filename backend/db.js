...
(async () => {
  try {
    ...
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_ventas_id_empleado ON Ventas(Id_Empleado)`);
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_ventas_id_cliente ON Ventas(Id_Cliente)`);
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_clientes_nombre ON Clientes(Nombres)`);
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_clientes_email ON Clientes(Correo)`);
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_empleados_nombre ON Empleados(Nombres)`);
    ...
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_detalles_ventas_id_venta ON DetallesDeVentas(Id_Venta)`);
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_detalles_ventas_id_producto ON DetallesDeVentas(Id_Producto)`);
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_detalles_compras_id_compra ON DetallesDeCompras(Id_Compra)`);
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_detalles_compras_id_producto ON DetallesDeCompras(Id_Producto)`);

    // Insertar usuario por defecto si no existe
    const hashedPassword = await bcrypt.hash('admin123', 10);
    await db.execute('INSERT OR IGNORE INTO Usuarios (NUsuario, Contraseña) VALUES (?, ?)', ['admin', hashedPassword]);
